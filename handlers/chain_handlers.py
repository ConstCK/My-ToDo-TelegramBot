...
async def select_task_category(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        text=f'Выбрана категория <b>{callback.data.split("_")[1]}</b> для добавления задачи.\n'
             ...
             f'Например: Задача №1-Краткое описание для этой задачи.'
...
async def create_task(message: Message, state: FSMContext):
    ...
    if task_data:
        ...
        if task.expire_at:
            scheduler.add_job(func=task_reminder, kwargs=({'task_id': task.id, 'chat_id': message.chat.id}),
                              name='expiring_tasks', trigger='date',
